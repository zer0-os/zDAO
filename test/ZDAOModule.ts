...
describe("ZDAOModule", function () {
  ...
  let zDAOModule: MockContract<ZDAOModule>,
    gnosisSafeProxy: MockContract<MockGnosisSafeProxy>,
    mockToken: MockToken;

  let proposalHash: BigNumber;

  beforeEach(async function () {
    [deployer, owner, executor, userA] = await ethers.getSigners();

    proposalHash = BigNumber.from(
      ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["string"], ["ProposalId"]))
    );

    const MockGnosisSafeProxyFactory = (await smock.mock<MockGnosisSafeProxy__factory>(
      "MockGnosisSafeProxy"
    )) as MockContractFactory<MockGnosisSafeProxy__factory>;
    gnosisSafeProxy =
      (await MockGnosisSafeProxyFactory.deploy()) as MockContract<MockGnosisSafeProxy>;
    ...
    zDAOModule = (await ZDAOModuleFactory.deploy()) as MockContract<ZDAOModule>;
    await zDAOModule.__ZDAOModule_init(gnosisSafeProxy.address, deployer.address);
  ...
  it("Only callable by Gnosis Safe", async function () {
    await expect(
      zDAOModule
        .connect(deployer)
        .executeProposal(0, proposalHash, mockToken.address, userA.address, 100000)
    ).to.be.revertedWith("Only callable by GnosisSafe");
  ...
  it("Should transfer ERC20", async function () {
    ...
    await zDAOModule
      .connect(executor)
      .executeProposal(0, proposalHash, mockToken.address, userA.address, amount);
  ...
  it("Should transfer ETH", async function () {
    ...
    await zDAOModule
      .connect(executor)
      .executeProposal(0, proposalHash, ethers.constants.AddressZero, userA.address, amount);
  ...
  it("Proposal should be executed", async function () {
    ...
    const isExecuted = await zDAOModule.isProposalExecuted(0, proposalHash);
