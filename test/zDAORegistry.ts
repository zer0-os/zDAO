...
import { IZNSHub, ZDAORegistry, ZDAORegistryV2, ZDAORegistryV2__factory, ZDAORegistry__factory } from '../types';
...
describe("zDAORegistry", async () => {
  ...
  let zNSHub: FakeContract<IZNSHub>,
    zDAORegistry: ZDAORegistry, zDAORegistryV2: ZDAORegistryV2;
  ...
  const wilder_kicks = ethers.BigNumber.from("0x79e5bdb3f024a898df02a5472e6fc5373e6a3c5f65317f58223a579d518378df");
  ...
  const upgradeToV2 = async (zDAORegistry: string) => {
    ...
    const zDAORegistryV2 = await upgrades.upgradeProxy(zDAORegistry, zDAORegistryFactory) as ZDAORegistryV2;
  ...
  describe("Add DAO", async () => {
   ...
   it("Should revert when add duplicated ENS name", async () => {
    ...
    await expect(zDAORegistry.connect(deployer).addNewDAO(ENS, gnosisSafe2)).to.be.revertedWith("ENS already has zDAO");
   });
  ...
  describe("zNA Association/Disassociation", async () => {
    ...
    describe("Association", async () => {
      it("Only zNA owner can associate zDAO and zNA", async () => {
        await expect(zDAORegistry.connect(zNAOwner).addZNAAssociation(1, wilder_beasts)).to.be.not.reverted;
      });
  
      it("Should revert when associate by non zNA owner", async () => {
        await expect(zDAORegistry.connect(userA).addZNAAssociation(1, wilder_beasts)).to.be.reverted;
      });
    });
    ...
    describe("Disassociation", async () => {
      ...
      it("Only zNA owner can disassociate zDAO and zNA", async () => {
        await expect(zDAORegistry.connect(zNAOwner).removeZNAAssociation(1, wilder_beasts)).to.be.not.reverted;
      });
  
      it("Should revert when disassociate by non-zNA owner", async () => {
        await expect(zDAORegistry.connect(userA).removeZNAAssociation(1, wilder_beasts)).to.be.reverted;
      });
  
      it("Should revert if already disassociated", async () => {
        await expect(zDAORegistry.connect(zNAOwner).removeZNAAssociation(1, wilder_beasts)).to.be.not.reverted;

        await expect(zDAORegistry.connect(zNAOwner).removeZNAAssociation(1, wilder_beasts)).to.be.revertedWith("zNA not associated");
      });
    });
  });
  ...
  describe("Upgradeability", async () => {
    ...
    it("Should add new DAO and associate with zNA", async () => {
      await expect(zDAORegistryV2.connect(deployer).addNewDAOWithToken(ENS2, gnosisSafe2, token)).to.be.not.reverted;
      await expect(zDAORegistryV2.connect(zNAOwner).addZNAAssociation(2, wilder_kicks)).to.be.not.reverted;
