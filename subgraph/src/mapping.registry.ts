...
export function handleDAOModified(event: DAOModified): void {
  const platformType = 0; // 0: Snapshot
  const zDAOId = event.params.zDAOId.toI32();
  const id = generateZDAORecordID(platformType, zDAOId);
...
export function handleDAOGnosisSafeModified(event: DAOGnosisSafeModified): void {
  const platformType = 0; // 0: Snapshot
  const zDAOId = event.params.zDAOId.toI32();
  const id = generateZDAORecordID(platformType, zDAOId);

  log.info("handleDAOGnosisSafeModified, called {}", [id]);
  ...
  if (zDAO) {
    ...
    log.error("handleDAOGnosisSafeModified, Unable to load ZDAORecord with zDAOId {}, {}", [
      id,
      event.block.number.toString(),
    ]);
  }
